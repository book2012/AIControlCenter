...
if [[ "$(/usr/bin/id -un)" != "$RUN_USER" ]]; then
  echo "Unexpected scheduler runtime user" >&2
  exit 78
fi
...
if [[ ! -d "$RUNTIME_TARGET" || ! -d "$VENV_ROOT" || -L "$VENV_ROOT" ]]; then
  echo "AIControlCenter scheduler runtime is unavailable" >&2
  exit 78
fi
...
if [[ "${RUNTIME_REAL%/*}" != "$VENV_ROOT_REAL" ]]; then
  echo "Scheduler runtime escaped runtime venv root" >&2
  exit 78
fi

RUNTIME_ID="$(/usr/bin/basename "$RUNTIME_REAL")"
if ! printf '%s\n' "$RUNTIME_ID" | /usr/bin/grep -Eq '^[0-9a-f]{12}$'; then
  echo "Scheduler runtime release identity is invalid" >&2
  exit 78
fi
...
if [[ ! -x "$PYTHON_PATH" || ! -d "$SOURCE_ROOT" || -L "$SOURCE_ROOT" ]]; then
  echo "Scheduler immutable runtime/source is unavailable" >&2
  exit 78
fi
if [[ ! -f "$SOURCE_MARKER" || ! -f "$RUNTIME_MARKER" || ! -f "$VALIDATOR" || ! -f "$SOURCE_ROOT/core/scheduler/daemon.py" ]]; then
  echo "Scheduler immutable source identity is unavailable" >&2
  exit 78
fi
...
if ! printf '%s\n' "$SOURCE_COMMIT" | /usr/bin/grep -Eq '^[0-9a-f]{40}$'; then
  echo "Scheduler source commit metadata is invalid" >&2
  exit 78
fi
if [[ "$RUNTIME_COMMIT" != "$SOURCE_COMMIT" ]]; then
  echo "Scheduler runtime/source identity mismatch" >&2
  exit 78
fi

unset PYTHONPATH
"$PYTHON_PATH" -P -B "$VALIDATOR" validate \
...
cd "$SOURCE_ROOT"
exec "$PYTHON_PATH" -P -B -m core.scheduler.daemon
