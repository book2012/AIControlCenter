"""Fixed Mac 01G1C operator. Importing this module performs no observation or write."""
...
from core.shopping import wordpress_generation_reconciliation as contract
from core.shopping.wordpress_generation_authorization import ConsumptionFailure, validate_consumption_result
...
# Select fields at Docker, never retrieve Config.Env, logs, healthcheck output or headers.
CONTAINER_FORMAT = (
    '{"id":{{json .Id}},"started":{{json .State.StartedAt}},'
    '"restart_count":{{json .RestartCount}},"running":{{json .State.Running}},'
    '"healthy":{{json .State.Health.Status}},"paused":{{json .State.Paused}},'
...
def _container(name):
    ...
    contract.require(value["healthy"] == "healthy")
    value["healthy"] = True
...
def _prepare_mutation():
    ...
    executable = trusted._trusted_compose_executable()
    trusted._trusted_docker_executable()
    command = (executable, *invocation.argv[1:3], *invocation.argv[4:])
...
def run():
    ...
    try:
        home = resolve_trusted_mac_account_home()
        owner = issue_trusted_ownership_expectation(home)
        authorization = WordPressGenerationAuthorizationStore.open_existing()
