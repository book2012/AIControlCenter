"""Issuer-created, irreversible SQLite store for fixed WordPress existing-generation-start authority."""
...
from core.shopping.wordpress_existing_generation_start_authorization import (
    AuthorizationConsumptionState, ConsumptionFailure, AuthorizationError, ConsumptionState, WordPressExistingGenerationStartAuthorization,
...
APPLICATION_ID = 0x57314731
USER_VERSION = 1
_COMPONENTS = ("Library", "Application Support", "AIControlCenter", "authorization", "wordpress-existing-generation-start-01g1g-authorization.sqlite3")
_FIELDS = tuple(WordPressExistingGenerationStartAuthorization.__dataclass_fields__)
_COLUMNS = ",".join(_FIELDS)
_DDL = """
CREATE TABLE wordpress_existing_generation_start_authorizations (
 ...
 authoritative_work_item TEXT NOT NULL CHECK(authoritative_work_item='SHOP-SERVICE-START-01G1G'),
 mutation_id TEXT NOT NULL CHECK(mutation_id='SHOP-SERVICE-START-01G1G:WORDPRESS_EXISTING_GENERATION_START'),
...
class WordPressExistingGenerationStartStoreError(RuntimeError): pass
...
def _validate(db):
    try:
        if (db.execute("PRAGMA application_id").fetchone()[0], db.execute("PRAGMA user_version").fetchone()[0]) != (APPLICATION_ID, USER_VERSION) or _fingerprint(db) != _EXPECTED:
            raise WordPressExistingGenerationStartStoreError("foreign or corrupt schema")
        if tuple(db.execute("PRAGMA integrity_check")) != (("ok",),):
            raise WordPressExistingGenerationStartStoreError("database integrity failure")
    except sqlite3.DatabaseError as exc:
        raise WordPressExistingGenerationStartStoreError("schema validation failed") from exc

class WordPressExistingGenerationStartAuthorizationStore:
    ...
    def _safe(path, test):
        if not path.is_absolute() or ".." in path.parts or path.is_symlink() or (not test and path.parts[-len(_COMPONENTS):] != _COMPONENTS):
            raise WordPressExistingGenerationStartStoreError("unsafe database path")
        for parent in (path.parent, *path.parents):
            if parent.exists() and parent.is_symlink(): raise WordPressExistingGenerationStartStoreError("symlink path rejected")
    ...
    def _require(path, uid, gid, mode, directory):
        ...
        if (not directory and meta.st_nlink != 1) or not kind or stat.S_IMODE(meta.st_mode)!=mode or (meta.st_uid,meta.st_gid)!=(uid,gid):
            raise WordPressExistingGenerationStartStoreError("unsafe path ownership or mode")
    ...
    def _setup(self, path, uid, gid, fault, test):
        ...
        with self._write() as db:
            if not _fingerprint(db):
                if (db.execute("PRAGMA application_id").fetchone()[0],db.execute("PRAGMA user_version").fetchone()[0])!=(0,0): raise WordPressExistingGenerationStartStoreError("foreign empty database")
    ...
    def _open_readonly(self,path,uid,gid,test):
        ...
        with self._read() as db:
            _validate(db); rows=db.execute("SELECT * FROM wordpress_existing_generation_start_authorizations WHERE state='AVAILABLE' AND expires_at>?",(datetime.now(timezone.utc).isoformat(),)).fetchall()
    ...
    def _issue(self, authorization):
        ...
        with self._write() as db:
            ...
            if db.execute("SELECT 1 FROM wordpress_existing_generation_start_authorizations").fetchone(): raise AuthorizationError("mutation already issued; reissuance forbidden")
            placeholders=",".join("?" for _ in _FIELDS)
            db.execute(f"INSERT INTO wordpress_existing_generation_start_authorizations ({_COLUMNS},state,claimed_at,committed_at) VALUES ({placeholders},'AVAILABLE',NULL,NULL)",tuple(getattr(authorization,n) for n in _FIELDS)); db.commit()
    ...
    def _consume(self, progress, observe):
        ...
        try:
            with self._write() as db:
                _validate(db); db.execute("BEGIN IMMEDIATE"); rows=db.execute("SELECT * FROM wordpress_existing_generation_start_authorizations WHERE state='AVAILABLE' AND expires_at>?",(now.isoformat(),)).fetchall()
                if len(rows)!=1: raise AuthorizationError("exactly one available authorization required")
                ...
                from core.shopping.wordpress_existing_generation_start_reconciliation import canonical_snapshot
                if canonical_snapshot(observe()) != authorization.precondition_json:
                    raise AuthorizationError("authorization-bound identity drift")
                validate_authorization(authorization, now=datetime.now(timezone.utc), uid=self._uid, gid=self._gid)
                if db.execute("UPDATE wordpress_existing_generation_start_authorizations SET state='DURABLY_CLAIMED',claimed_at=? WHERE authorization_id=? AND state='AVAILABLE'",(now.isoformat(),authorization.authorization_id)).rowcount!=1: raise AuthorizationError("authorization claim lost")
        ...
        except (sqlite3.DatabaseError,WordPressExistingGenerationStartStoreError) as exc: raise WordPressExistingGenerationStartStoreError("durable claim failed closed") from exc
        ...
        try:
            with self._write() as db:
                ...
                if db.execute("UPDATE wordpress_existing_generation_start_authorizations SET state='COMMITTED',committed_at=? WHERE authorization_id=? AND state='DURABLY_CLAIMED'",(committed_at,authorization.authorization_id)).rowcount!=1: raise WordPressExistingGenerationStartStoreError("claim inconsistent")
                self._inject("before_final_commit",db); attempted=True; db.commit(); self._inject("after_final_commit",db)
        except (sqlite3.DatabaseError,WordPressExistingGenerationStartStoreError) as exc:
            if not attempted or not self._exact(authorization,committed_at): raise WordPressExistingGenerationStartStoreError("final commit failed closed") from exc
        if not self._exact(authorization,committed_at): raise WordPressExistingGenerationStartStoreError("committed read-back failed")
        return self._result(authorization)

    def _exact(self,authorization,committed_at):
        try:
            with self._read() as db: _validate(db); row=db.execute("SELECT * FROM wordpress_existing_generation_start_authorizations WHERE authorization_id=?",(authorization.authorization_id,)).fetchone()
            if not row:
                return False
    ...
    def _authorization(row):
        ...
        if type(fields["maximum_uses"]) is not int or fields["maximum_uses"] != 1:
            raise WordPressExistingGenerationStartStoreError("invalid maximum uses in durable row")
        for name in DENIED:
            if type(fields[name]) is not int or fields[name] != 0:
                raise WordPressExistingGenerationStartStoreError(
                    f"invalid {name} in durable row"
                )
            fields[name] = False
        value=object.__new__(WordPressExistingGenerationStartAuthorization)
        for name in _FIELDS:
            object.__setattr__(value, name, fields[name])
        return value
    @staticmethod
    def _result(auth):
        receipt=object.__new__(WordPressExistingGenerationStartConsumptionReceipt)
        for name in WordPressExistingGenerationStartConsumptionReceipt.__dataclass_fields__: object.__setattr__(receipt,name,ConsumptionState.COMMITTED if name=="state" else getattr(auth,name))
        result=object.__new__(WordPressExistingGenerationStartConsumptionResult); object.__setattr__(result,"receipt",receipt); return result

__all__=("WordPressExistingGenerationStartAuthorizationStore","WordPressExistingGenerationStartStoreError")
