"""Issuer-created, irreversible SQLite store for fixed Colima profile authority."""
...
from core.shopping.colima_lifecycle_authorization import (
    AuthorizationConsumptionState, ConsumptionFailure, AuthorizationError, ConsumptionState, ColimaLifecycleAuthorization,
    ColimaLifecycleConsumptionReceipt, ColimaLifecycleConsumptionResult,
    validate_authorization,
)

APPLICATION_ID = 0x43473146
USER_VERSION = 1
_COMPONENTS = ("Library", "Application Support", "AIControlCenter", "authorization", "colima-lifecycle-01g1f-authorization.sqlite3")
_FIELDS = tuple(ColimaLifecycleAuthorization.__dataclass_fields__)
_COLUMNS = ",".join(_FIELDS)
_DDL = """
CREATE TABLE colima_lifecycle_authorizations (
 ...
 authoritative_work_item TEXT NOT NULL CHECK(authoritative_work_item='SHOP-SERVICE-START-01G1F'),
 mutation_id TEXT NOT NULL CHECK(mutation_id='SHOP-SERVICE-START-01G1F:COLIMA_RUNTIME_LIFECYCLE_RECONCILIATION'),
...
class ColimaLifecycleStoreError(RuntimeError): pass
...
def _validate(db):
    try:
        if (db.execute("PRAGMA application_id").fetchone()[0], db.execute("PRAGMA user_version").fetchone()[0]) != (APPLICATION_ID, USER_VERSION) or _fingerprint(db) != _EXPECTED:
            raise ColimaLifecycleStoreError("foreign or corrupt schema")
        if tuple(db.execute("PRAGMA integrity_check")) != (("ok",),):
            raise ColimaLifecycleStoreError("database integrity failure")
    except sqlite3.DatabaseError as exc:
        raise ColimaLifecycleStoreError("schema validation failed") from exc

class ColimaLifecycleAuthorizationStore:
    ...
    def _safe(path, test):
        if not path.is_absolute() or ".." in path.parts or path.is_symlink() or (not test and path.parts[-len(_COMPONENTS):] != _COMPONENTS):
            raise ColimaLifecycleStoreError("unsafe database path")
        for parent in (path.parent, *path.parents):
            if parent.exists() and parent.is_symlink(): raise ColimaLifecycleStoreError("symlink path rejected")
    ...
    def _require(path, uid, gid, mode, directory):
        ...
        if not kind or stat.S_IMODE(meta.st_mode)!=mode or (meta.st_uid,meta.st_gid)!=(uid,gid):
            raise ColimaLifecycleStoreError("unsafe path ownership or mode")
    ...
    def _setup(self, path, uid, gid, fault, test):
        ...
        with self._write() as db:
            if not _fingerprint(db):
                if (db.execute("PRAGMA application_id").fetchone()[0],db.execute("PRAGMA user_version").fetchone()[0])!=(0,0): raise ColimaLifecycleStoreError("foreign empty database")
    ...
    def _open_readonly(self,path,uid,gid,test):
        ...
        with self._read() as db:
            _validate(db); rows=db.execute("SELECT * FROM colima_lifecycle_authorizations WHERE state='AVAILABLE' AND expires_at>?",(datetime.now(timezone.utc).isoformat(),)).fetchall()
    ...
    def _issue(self, authorization):
        ...
        with self._write() as db:
            ...
            if db.execute("SELECT 1 FROM colima_lifecycle_authorizations").fetchone(): raise AuthorizationError("outstanding usable authorization exists")
            placeholders=",".join("?" for _ in _FIELDS)
            db.execute(f"INSERT INTO colima_lifecycle_authorizations ({_COLUMNS},state,claimed_at,committed_at) VALUES ({placeholders},'AVAILABLE',NULL,NULL)",tuple(getattr(authorization,n) for n in _FIELDS)); db.commit()
    ...
    def _consume(self, progress, observe):
        ...
        try:
            with self._write() as db:
                _validate(db); db.execute("BEGIN IMMEDIATE"); rows=db.execute("SELECT * FROM colima_lifecycle_authorizations WHERE state='AVAILABLE' AND expires_at>?",(now.isoformat(),)).fetchall()
                if len(rows)!=1: raise AuthorizationError("exactly one available authorization required")
                ...
                from core.shopping.colima_lifecycle_reconciliation import canonical_snapshot
                if canonical_snapshot(observe()) != authorization.precondition_json:
                    raise AuthorizationError("authorization-bound identity drift")
                validate_authorization(authorization, now=datetime.now(timezone.utc), uid=self._uid, gid=self._gid)
                if db.execute("UPDATE colima_lifecycle_authorizations SET state='DURABLY_CLAIMED',claimed_at=? WHERE authorization_id=? AND state='AVAILABLE'",(now.isoformat(),authorization.authorization_id)).rowcount!=1: raise AuthorizationError("authorization claim lost")
        ...
        except (sqlite3.DatabaseError,ColimaLifecycleStoreError) as exc: raise ColimaLifecycleStoreError("durable claim failed closed") from exc
        ...
        try:
            with self._write() as db:
                ...
                if db.execute("UPDATE colima_lifecycle_authorizations SET state='COMMITTED',committed_at=? WHERE authorization_id=? AND state='DURABLY_CLAIMED'",(committed_at,authorization.authorization_id)).rowcount!=1: raise ColimaLifecycleStoreError("claim inconsistent")
                self._inject("before_final_commit",db); attempted=True; db.commit(); self._inject("after_final_commit",db)
        except (sqlite3.DatabaseError,ColimaLifecycleStoreError) as exc:
            if not attempted or not self._exact(authorization,committed_at): raise ColimaLifecycleStoreError("final commit failed closed") from exc
        if not self._exact(authorization,committed_at): raise ColimaLifecycleStoreError("committed read-back failed")
        return self._result(authorization)

    def _exact(self,authorization,committed_at):
        try:
            with self._read() as db: _validate(db); row=db.execute("SELECT * FROM colima_lifecycle_authorizations WHERE authorization_id=?",(authorization.authorization_id,)).fetchone()
            if not row:
                return False
            expected = [getattr(authorization, name) for name in _FIELDS]
            for name in ("lifecycle_authority", "production_authority", "ubuntu_authority"):
                expected[_FIELDS.index(name)] = int(name == "lifecycle_authority")
            return bool(
    ...
    def _authorization(row):
        ...
        if type(fields["maximum_uses"]) is not int or fields["maximum_uses"] != 1:
            raise ColimaLifecycleStoreError("invalid maximum uses in durable row")
        for name in ("lifecycle_authority", "production_authority", "ubuntu_authority"):
            if type(fields[name]) is not int or fields[name] != int(name == "lifecycle_authority"):
                raise ColimaLifecycleStoreError(
                    f"invalid {name} in durable row"
                )
            fields[name] = name == "lifecycle_authority"
        value=object.__new__(ColimaLifecycleAuthorization)
        for name in _FIELDS:
            object.__setattr__(value, name, fields[name])
        return value
    @staticmethod
    def _result(auth):
        receipt=object.__new__(ColimaLifecycleConsumptionReceipt)
        for name in ColimaLifecycleConsumptionReceipt.__dataclass_fields__: object.__setattr__(receipt,name,ConsumptionState.COMMITTED if name=="state" else getattr(auth,name))
        result=object.__new__(ColimaLifecycleConsumptionResult); object.__setattr__(result,"receipt",receipt); return result

__all__=("ColimaLifecycleAuthorizationStore","ColimaLifecycleStoreError")
