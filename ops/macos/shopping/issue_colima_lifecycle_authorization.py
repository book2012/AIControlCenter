"""Separate interactive 01G1F issuer; issuance never executes the mutation."""
...
from core.shopping.colima_lifecycle_authorization import ColimaLifecycleAuthorization, immutable_contract
from core.shopping.colima_lifecycle_reconciliation import MUTATION_ID, LIFECYCLE_ARGS, canonical_snapshot, projection
from ops.macos.shopping.colima_lifecycle_authorization_store import ColimaLifecycleAuthorizationStore
from ops.macos.shopping.colima_lifecycle_operator import observe_preconditions
...
def issue():
    ...
    if json.loads(before)["declared"]["head"] != expected_head:
    ...
    print(json.dumps({**immutable_contract(uid=owner.expected_uid, gid=owner.expected_gid),
                      ...
                      "mutation_argv": ["/opt/homebrew/bin/colima", *LIFECYCLE_ARGS],
    ...
    ColimaLifecycleAuthorizationStore._initialize_for_issuer()._issue(authorization)
