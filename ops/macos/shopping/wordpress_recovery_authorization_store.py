...
from core.shopping.wordpress_recovery_authorization import (
...
APPLICATION_ID = 0x57473144
USER_VERSION = 1
_COMPONENTS = ("Library", "Application Support", "AIControlCenter", "authorization", "wordpress-generation-01g1d-authorization.sqlite3")
...
CREATE TABLE wordpress_mutation_authorizations (
 ...
 authoritative_work_item TEXT NOT NULL CHECK(authoritative_work_item='SHOP-SERVICE-START-01G1D'),
 environment TEXT NOT NULL CHECK(environment='CONTROLLED_NON_PRODUCTION'),
 mutation_id TEXT NOT NULL CHECK(mutation_id='SHOP-SERVICE-START-01G1D:WORDPRESS_RUNTIME_GENERATION_RECOVERY'),
 ...
 expected_before_binding TEXT NOT NULL CHECK(expected_before_binding='127.0.0.1:58082->80/tcp'),
...
class WordPressRecoveryAuthorizationStore:
    ...
    def _issue(self, authorization):
        ...
        with self._write() as db:
            ...
            if db.execute("SELECT 1 FROM wordpress_mutation_authorizations").fetchone(): raise AuthorizationError("outstanding usable authorization exists")
    ...
    def _consume(self, progress, observe):
        ...
        try:
            with self._write() as db:
                ...
                from core.shopping.wordpress_recovery_reconciliation import canonical_snapshot
...
__all__=("WordPressRecoveryAuthorizationStore","WordPressAuthorizationStoreError")
