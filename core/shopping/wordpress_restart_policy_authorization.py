"""Immutable one-shot authority for the dedicated 01G1F0 WordPress restart-policy reconciliation."""
...
from core.shopping.wordpress_restart_policy_reconciliation import (
...
class WordPressRestartPolicyAuthorization:
...
@dataclass(frozen=True, slots=True, init=False)
class WordPressRestartPolicyConsumptionReceipt:
    authorization_id: str; issued_at: str; expires_at: str
    trusted_uid: int; trusted_gid: int
    authoritative_work_item: str; mutation_id: str; profile: str; profile_file: str
    precondition_json: str
...
@dataclass(frozen=True, slots=True, init=False)
class WordPressRestartPolicyConsumptionResult:
    receipt: WordPressRestartPolicyConsumptionReceipt
    def __new__(cls):
...
def validate_authorization(value: object, *, now: datetime, uid: int, gid: int) -> None:
    if type(value) is not WordPressRestartPolicyAuthorization:
        raise AuthorizationError("exact WordPress restart policy authorization type required")
    if any(
    ...
    from core.shopping.wordpress_restart_policy_reconciliation import parse_binding
    try:
...
def validate_consumption_result(value: object, *, now: datetime, uid: int, gid: int) -> WordPressRestartPolicyConsumptionReceipt:
    if type(value) is not WordPressRestartPolicyConsumptionResult or type(value.receipt) is not WordPressRestartPolicyConsumptionReceipt:
        raise AuthorizationError("exact structured WordPress restart policy consumption receipt required")
    receipt = value.receipt
    if receipt.state is not ConsumptionState.COMMITTED:
        raise AuthorizationError("receipt is not durably committed")
    authorization = object.__new__(WordPressRestartPolicyAuthorization)
    for name in WordPressRestartPolicyAuthorization.__dataclass_fields__:
...
__all__ = ("AuthorizationError", "ConsumptionState", "WordPressRestartPolicyAuthorization",
