"""Pure, isolated 01G1G existing-generation-start contract; metadata continuity only."""
import json
import re
...
AUTHORITATIVE_WORK_ITEM = 'SHOP-SERVICE-START-01G1G'
MUTATION_ID = AUTHORITATIVE_WORK_ITEM + ':WORDPRESS_EXISTING_GENERATION_START'
...
MUTATION_ARGS = ('--context', CONTEXT, 'start', FAILED_ID)
...
def validate_snapshot(v, *, post=False):
    ...
    keys(v['wordpress'], 'id status running restart_count restart maximum_retry_count healthy')
    require(v['wordpress']['healthy'] in (('starting', 'healthy', 'unhealthy') if post else (None, 'starting', 'healthy', 'unhealthy')))
    require({k: value for k, value in v['wordpress'].items() if k != 'healthy'} ==
        dict(id=FAILED_ID, status='running' if post else 'created', running=post,
             restart_count=0, restart='no', maximum_retry_count=0))
    require(v['wordpress']['running'] is post)
...
def validate_post(before, after):
    ...
    normalized['wordpress'] = before['wordpress'].copy()
