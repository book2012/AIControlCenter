"""Immutable one-shot authority for the dedicated 01G1E Colima bind-source projection reconciliation."""
...
class ColimaProjectionAuthorization:
...
@dataclass(frozen=True, slots=True, init=False)
class ColimaProjectionConsumptionReceipt:
    authorization_id: str; issued_at: str; expires_at: str
    trusted_uid: int; trusted_gid: int
    authoritative_work_item: str; mutation_id: str; profile: str; profile_file: str
    precondition_json: str
...
@dataclass(frozen=True, slots=True, init=False)
class ColimaProjectionConsumptionResult:
    receipt: ColimaProjectionConsumptionReceipt
    def __new__(cls):
...
def validate_authorization(value: object, *, now: datetime, uid: int, gid: int) -> None:
    if type(value) is not ColimaProjectionAuthorization:
        raise AuthorizationError("exact Colima projection authorization type required")
    if any(
    ...
    from core.shopping.colima_projection_reconciliation import parse_binding
    try:
...
def validate_consumption_result(value: object, *, now: datetime, uid: int, gid: int) -> ColimaProjectionConsumptionReceipt:
    if type(value) is not ColimaProjectionConsumptionResult or type(value.receipt) is not ColimaProjectionConsumptionReceipt:
        raise AuthorizationError("exact structured Colima projection consumption receipt required")
...
__all__ = ("AuthorizationError", "ConsumptionState", "ColimaProjectionAuthorization",
