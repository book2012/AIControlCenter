"""Immutable one-shot authority for the dedicated 01G1C WordPress generation reconciliation."""
...
from core.shopping.wordpress_generation_reconciliation import (
...
def validate_authorization(value: object, *, now: datetime, uid: int, gid: int) -> None:
    ...
    from core.shopping.wordpress_generation_reconciliation import parse_binding
    try:
        bound = parse_binding(value.precondition_json)
        if (bound["source_metadata"]["st_uid"], bound["source_metadata"]["st_gid"]) != (uid, gid):
