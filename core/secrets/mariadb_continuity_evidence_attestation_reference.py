"""Closed, value-free contract for external evidence attestation references."""
...
class EvidenceReferenceIdentityClass(str, Enum):
    """Repository-defined identities; never caller-provided identifiers."""

    AUTH_PLUGIN_HISTORICAL_ATTESTATION = "AUTH_PLUGIN_HISTORICAL_ATTESTATION"
    PYMYSQL_COMPATIBILITY_ATTESTATION = "PYMYSQL_COMPATIBILITY_ATTESTATION"
    DATA_IDENTITY_ATTESTATION = "DATA_IDENTITY_ATTESTATION"
    CONTINUITY_LINEAGE_ATTESTATION = "CONTINUITY_LINEAGE_ATTESTATION"
...
class SemanticsChangeRequired(str, Enum):
    NO = "NO"


@dataclass(frozen=True, slots=True)
class ExternalEvidenceAttestationReferenceContract:
    """Requirements and fail-closed facts, without evidence values or authority."""

    evidence_requirements: tuple[EvidenceRequirementCategory, ...] = field(
        default=tuple(EvidenceRequirementCategory), init=False
    )
    reference_identity_classes: tuple[EvidenceReferenceIdentityClass, ...] = field(
        default=tuple(EvidenceReferenceIdentityClass), init=False
    )
    reference_verification_state: VerificationState = field(
    ...
    immutable_integrity_binding_required: bool = field(default=True, init=False)
    timestamp_binding_required: bool = field(default=True, init=False)
    trusted_issuer_binding_required: bool = field(default=True, init=False)
    ...
    integrity_binding_satisfied: bool = field(default=False, init=False)
    account_baseline_binding_satisfied: bool = field(default=False, init=False)
    authority: bool = field(default=False, init=False)
    compatible: bool = field(default=False, init=False)
    reference_local_ready: bool = field(default=False, init=False)
...
def canonical_external_evidence_attestation_reference_contract(
) -> ExternalEvidenceAttestationReferenceContract:
    return ExternalEvidenceAttestationReferenceContract()
