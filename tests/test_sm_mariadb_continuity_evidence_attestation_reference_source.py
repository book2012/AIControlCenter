...
from ops.macos.shopping.mariadb_continuity_evidence_attestation_reference_source import (
    ExternalEvidenceAttestationReferenceSource,
    canonical_external_evidence_attestation_reference_source,
)


ROOT = Path(__file__).parents[1]
PRODUCTION_FILES = (
    ROOT / "core/secrets/mariadb_continuity_evidence_attestation_reference.py",
    ROOT / "ops/macos/shopping/mariadb_continuity_evidence_attestation_reference_source.py",
)
ACTIONS = {
...
def test_production_ast_has_no_io_network_database_sql_or_runtime_access():
    prohibited_imports = {
...
def test_projection_is_symbolic_value_free_and_zero_authority():
    source = canonical_external_evidence_attestation_reference_source()
    assert type(source.contract) is ExternalEvidenceAttestationReferenceContract
    ...
    assert all(projection[key] is False for key in (
        "evidence_exists", "provenance_valid", "integrity_binding_satisfied",
        "account_baseline_binding_satisfied", "authority", "compatible",
        "reference_local_ready", "five_category_data_identity_complete",
        "three_category_continuity_lineage_complete", "recover_evidence_sufficient",
    ))
    assert "production_validation_ready" not in projection
    for item in fields(source):
        with pytest.raises(TypeError):
            ExternalEvidenceAttestationReferenceSource(**{item.name: True})


def test_exact_six_actions_are_preserved_and_target_is_not_an_action():
