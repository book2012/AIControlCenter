...
from ops.macos.shopping.mariadb_continuity_evidence_admission_source import (
    ExternalEvidenceAdmissionSource,
    canonical_external_evidence_admission_source,
)


ROOT = Path(__file__).parents[1]
PRODUCTION_FILES = (
    ROOT / "core/secrets/mariadb_continuity_evidence_admission.py",
    ROOT / "ops/macos/shopping/mariadb_continuity_evidence_admission_source.py",
)
ACTIONS = {
...
def test_package_four_production_ast_has_zero_io_network_database_and_sql():
    prohibited_imports = {
...
def test_projection_is_mac_owned_symbolic_and_zero_authority():
    source = canonical_external_evidence_admission_source()
    assert type(source.contract) is ExternalEvidenceAdmissionContract
    ...
    assert all(
        ...
        for key in (
            "authorization_authority", "capability_authority",
            "execution_authority", "mutation_authority", "retry_authority",
            "reconnect_authority", "rollback_authority",
        )
    )
    for item in fields(source):
        with pytest.raises(TypeError):
            ExternalEvidenceAdmissionSource(**{item.name: True})


def test_exact_six_actions_and_target_only_semantics_are_unchanged():
