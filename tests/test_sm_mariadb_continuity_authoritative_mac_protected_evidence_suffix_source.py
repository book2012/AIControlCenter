...
from ops.macos.shopping.mariadb_continuity_authoritative_mac_protected_evidence_suffix_source import (
    MariaDBContinuityAuthoritativeMacProtectedEvidenceSuffixPolicySource,
    canonical_mariadb_continuity_authoritative_mac_protected_evidence_suffix_policy_source,
)


ROOT = Path(__file__).parents[1]
IMPLEMENTATION_FILES = (
    ROOT / "core/secrets/mariadb_continuity_authoritative_mac_protected_evidence_suffix.py",
    ROOT / "ops/macos/shopping/mariadb_continuity_authoritative_mac_protected_evidence_suffix_source.py",
)


def test_implementation_is_inert_without_runtime_path_or_external_io_capability():
    prohibited_imports = {"pathlib", "os", "pwd", "sys", "glob", "socket", "subprocess", "pymysql", "docker"}
    prohibited_calls = {"open", "exists", "stat", "lstat", "resolve", "realpath", "is_file", "is_dir", "is_symlink", "read", "read_text", "read_bytes", "glob", "rglob", "iterdir", "walk", "listdir", "scandir", "getenv", "getpwuid", "getuid", "getgid", "home"}
...
def test_source_factory_has_zero_path_inputs_and_projection_is_inert():
    factory = canonical_mariadb_continuity_authoritative_mac_protected_evidence_suffix_policy_source
    ...
    with pytest.raises(TypeError):
        MariaDBContinuityAuthoritativeMacProtectedEvidenceSuffixPolicySource(policy="caller-value")
    with pytest.raises(TypeError):
